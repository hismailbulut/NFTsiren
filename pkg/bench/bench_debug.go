...
func PrintResults(out io.Writer) {
	...
	// table.Render has multiple write calls which causes
	// multiple flushes to buffer and this broke output when
	// data send out network
	// in order to flush once, we first render it to our buffer and
	// then flush it to out
	table.Render()
	io.Copy(out, &buf)
}
